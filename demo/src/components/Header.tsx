        ...
        <header className="flex flex-col border-b border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900">
            ...
            <div className="px-6 pb-4 space-y-3">
                <div className="flex flex-col sm:flex-row gap-2 justify-center">
                    <a
                        href={getGhproxyApkDownloadUrl(version)}
                        target="_blank"
                        rel="noopener noreferrer"
                        onClick={handleOpenUrl(getGhproxyApkDownloadUrl(version))}
                    ...
                    <a
                        href={getGhproxyExeDownloadUrl(version)}
                        target="_blank"
                        rel="noopener noreferrer"
                        onClick={handleOpenUrl(getGhproxyExeDownloadUrl(version))}
                ...
                <div className="flex flex-wrap gap-x-4 gap-y-1 justify-center text-xs text-gray-500 dark:text-gray-400">
                    <a
                        href={getApkDownloadUrl(version)}
                        target="_blank"
                        rel="noopener noreferrer"
                        onClick={handleOpenUrl(getApkDownloadUrl(version))}
                    ...
                    <a
                        href={getExeDownloadUrl(version)}
                        target="_blank"
                        rel="noopener noreferrer"
                        onClick={handleOpenUrl(getExeDownloadUrl(version))}
                        className="hover:text-gray-700 dark:hover:text-gray-300 underline decoration-dotted"
                    >
                        GitHub EXE
                    </a>
